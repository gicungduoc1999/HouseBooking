...
public class AdditionalServiceDAO {
    ...
    public List<AdditionalService> getAdditionalServiceByHouseId(int houseId) {
        String sql = "select Additional_service.* , add_service_price \n"
                + "from Additional_service ,House_additional_service\n"
                + "where House_additional_service.add_service_id = Additional_service.add_service_id\n"
                + "and  House_additional_service.house_id = ? ";
        List<AdditionalService> list = new ArrayList<>();
        try {
            //tạo khay chứa câu lệnh
            PreparedStatement pre = con.prepareStatement(sql);
            ...
            while (resultSet.next()) {
                ...
                String servicedesc = resultSet.getString(3);
                AdditionalService as = new AdditionalService(serviceid, servicename, servicedesc);
                as.setPrice(resultSet.getFloat(4));
                list.add(as);
            }
        } catch (Exception e) {
            System.out.println("error: " + e);
